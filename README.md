...
- **Image preprocessing & classification**  
  Convert document images to PNG and classify them using a lightweight **MobileNetV2**.

- **Text extraction (OCR)**  
  Extract text from images using **Tesseract** with preprocessing (grayscale, thresholding).

- **Text embeddings & retrieval**  
  Embed extracted text using **Sentence Transformers (`paraphrase-mpnet-base-v2`)** and retrieve relevant information using cosine similarity.

- **RAG-style QA**  
  Generate answers to user queries by combining retrieved text with a **GPT-style generator** (optional) or rule-based extraction for invoices.

- **Evaluation metrics included**  
  Quantitative evaluation of each component to demonstrate reliability.
